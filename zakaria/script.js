...
function buildVCard(config) {
  ...
  if (contact?.phone2) lines.push(`TEL;TYPE=WORK:${contact.phone2}`);
...
function render(config, logoUrl) {
  ...
  const contact = {
    ...
    phone2: config.contact?.phone2?.trim?.() ?? config.contact?.phone2 ?? '',
  ...
  if (contact.phone2) quickActions.appendChild(quickBtn(`tel:${contact.phone2}`, 'phone', 'Call phone 2'));
  ...
  if (contact.phone2) {
    contactList.appendChild(
      contactBlock('Phone 2', formatPhoneDisplay(contact.phone2), 'phone', `tel:${contact.phone2}`),
    );
  }
  if (contact.email) {
    contactList.appendChild(
  ...
  const hasContact = contact.phone || contact.phone2 || contact.email || contact.website || contact.location;
